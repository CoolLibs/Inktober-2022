...
INPUT float    Gradient_Size;
INPUT float    Time_speed;
INPUT float    Irregularity_rarity;
...
void main()
{
    vec2 uv = normalized_uv() * Scale + 1000.;
    vec3 color;
    ...
    float grad = (-hex_id.x - hex_id.y - _time * Time_speed) * Gradient_Size;
    float rand = pow(snoise(vec3(hex_id, _time)), Irregularity_rarity);
    grad       = max(grad, rand);
    ...
    color += mix(Background, Gradient(grad).rgb, is_hex);
