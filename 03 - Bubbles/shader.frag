...
float is_highlight(CircleCoords coords)
{
    ...
    angular_pos       = (angular_pos - 0.5) * (angular_pos - 0.5) * 2; // Map 0 to 1 and 1 to 1
    ...
    //     Highlight_length
    // );
    float thickness = pow(angular_pos, Highlight_shape) * Highlight_length;
    ...
    // float is_highlight = 1. - smoothstep(highlight_min, highlight_max, coords.radial);
...
void main()
{
    ...
    vec2 Inv_center = vec2(
        0.,
        mix(-20., 18., smoothstep(0., 25., _time))
    );
